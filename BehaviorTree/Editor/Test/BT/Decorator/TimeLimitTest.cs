    ...
    public class TimeLimitTest : Test
    {
        ...
        public void Timeout()
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                var sut = new TimeLimit(rndTime, child);
                var bt = CreateBehaviorTree(sut);
                bt.Start();

                Assert.AreEqual(sut.CurrentState, Node.State.ACTIVE, "TimeLimit AVTIVE");
                Assert.AreEqual(child.CurrentState, Node.State.ACTIVE, "Start, child should be ACTIVED");
        ...
        public void Child_Return_First()
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                Assert.AreEqual(child.CurrentState, Node.State.INACTIVE, "Child finished, child should be INACTIVED");
