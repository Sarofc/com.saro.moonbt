...
namespace Saro.BT.Designer
{
     ...
     *
     *      // TODO
     *      x. copy paste 连线，增加专用copy数据类，保存所有节点，和连线，使用数组index来索引吧
     *      2. 连线/断线/移动节点 时，更新preOrder
     */
    public class BTGraphView : AbstractGraphView
    {
        ...
        public BTGraphView(BehaviorTree tree)
        {
            ...
            //graphViewChanged += OnGraphChange; // 只能拦截改动，shadergraph是拦截后，手动apply上去
        }
        ...
        {
            if (Application.isPlaying) return; // 运行时屏蔽
